...
function readOpenClawConfig(configPath: string): OpenClawConfig {
  const content = fs.readFileSync(expandPath(configPath), 'utf-8');
...
function writeOpenClawConfig(configPath: string, config: OpenClawConfig): void {
  fs.writeFileSync(expandPath(configPath), JSON.stringify(config, null, 2), 'utf-8');
}
...
function createAgentWorkspace(workspacePath: string): void {
  const resolved = expandPath(workspacePath);
  const dirs = [
    resolved,
    path.join(resolved, 'memory'),
  ];
...
/**
 * 与 openclaw.json agents.list 中 default: true 项一致；无标记时取列表第一项。
 */
function resolveDefaultAgentIdFromList(agents: AgentConfig[]): string {
  const def = agents.find(a => a.default === true);
  return def?.id ?? agents[0]?.id ?? 'steward';
}
...
function generateAgentsTemplate(existingAgents: AgentConfig[]): string {
  const defaultAgentId = resolveDefaultAgentIdFromList(existingAgents);
  ...
  try {
    ...
    for (const agent of agents) {
      try {
        ...
        const workspacePath = `~/.openclaw/workspace-${agent.agentId}`;
        const agentDirPath = `~/.openclaw/agents/${agent.agentId}/agent`;
        ...
        // 创建工作区目录（expandPath 在 createAgentWorkspace 内部处理）
        createAgentWorkspace(workspacePath);
        fs.mkdirSync(expandPath(agentDirPath), { recursive: true });
        
        // 生成 Agent 人设文件
        const wsResolved = expandPath(workspacePath);
        const soulPath = path.join(wsResolved, 'SOUL.md');
        const agentsPath = path.join(wsResolved, 'AGENTS.md');
        const userPath = path.join(wsResolved, 'USER.md');
...
例如：
- \`3 个\` - 我推荐：steward（大总管）+ dev（开发）+ content（内容）
- \`6 个\` - 完整团队：全部 6 个角色
- \`自定义\` - 你自由选择角色
...
例如：\`3\` 或 \`6\``);
        ...
        if (numCount === 1) {
          recommendedAgents = '推荐：**steward**（大总管）- 全能型助理';
        } else if (numCount === 2) {
          recommendedAgents = '推荐：**steward**（大总管）+ **dev**（开发助理）';
        } else if (numCount === 3) {
          recommendedAgents = '推荐：**steward**（大总管）+ **dev**（开发助理）+ **content**（内容助理）';
        } else if (numCount === 6) {
          recommendedAgents = '推荐：完整 6 人团队 - steward + dev + content + ops + law + finance';
        } else {
